...
import type { PersistStrategy } from '#/pinia';
...
export const createPersistPlugin = ({
  ...
  const updateStorage = (strategy: PersistStrategy, store: Store) => {
    ...
    if (strategy.paths) {
      const partialState = strategy.paths.reduce((finalObj: AnyObject, key: string) => {
  ...
  const plugin = ({ options, store }: PiniaPluginContext): void => {
    ...
    strategies.forEach(strategy => {
      ...
      if (storageResult) {
        const result =
          typeof storageResult === 'string' ? JSON.parse(storageResult) : storageResult;
